...
func (VBlockMode) Handle(ctx *Context, k motion.Key) {
	...
	if cmd, ok := ctx.Commands.Lookup(k); ok {
		slog.Debug("VBlock Mode | Trying to execute command", slog.String("cmd", fmt.Sprintf("%T", cmd)))
		err := cmd.Execute(ctx)
		if err != nil {
			slog.Error("VBlock Mode | Failed to execute command", slog.String("cmd", fmt.Sprintf("%T", cmd)))
		}
		return
	}
