...
func (z *Zone) DrawCommandZone(appAssets *assets.Assets, c *SpreadsheetCursor) {
	const textSpacing float32 = 4
	var statusLineColor rl.Color = c.Mode.Color()
	// Status Line
	rl.DrawRectangle(int32(z.Bounds.X), int32(z.Bounds.Y), int32(z.Bounds.Width), int32(z.Bounds.Height/2), colors.Mantle())
	var modeStatusText string = c.Mode.String()
	...
	var detailsStatusText string = fmt.Sprintf("%d/%d | %d/%d | %d%%", c.Col+1, c.MaxCol+1, c.Row+1, c.MaxRow+1, 0)
	...
	c.Logs.CheckForMessage()
	appAssets.DrawTextMainFont(c.Logs.LastMessage, rl.Vector2{X: z.Bounds.X + textSpacing, Y: z.Bounds.Y + z.Bounds.Height/2 + textSpacing/2}, colors.Text())

	var motionBufWidth float32 = appAssets.MeasureTextMainFont(c.MotionBuf).X + textSpacing*8
	appAssets.DrawTextMainFont(c.MotionBuf, rl.Vector2{X: z.Bounds.Width - z.Bounds.X - motionBufWidth, Y: z.Bounds.Y + z.Bounds.Height/2 + textSpacing/2}, colors.Text())
}
